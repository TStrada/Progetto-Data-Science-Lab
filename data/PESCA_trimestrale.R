...
d<-read.csv("pesca_trimestrale.csv", sep=",")

d$year<-as.Date(d$year)

#Creazione training set PESCA TRIMESTRALE:
ds<-d$year[25:32]
y<-d$revenue[25:32]
...
plot(m,forecast)+scale_y_continuous(n.breaks =10)+ labs(y="revenue",x="year")+add_changepoints_to_plot(m)
